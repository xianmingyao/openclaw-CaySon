...
### 问题2：delivery.channel 未指定（已修复）
**问题根因**：配置了多个渠道（feishu + openclaw-weixin），announce 模式需要明确指定 channel

**修复方法**：
```bash
openclaw cron update <job-id> --patch '{"delivery": {"mode": "announce", "channel": "openclaw-weixin"}}'
```

**实际错误信息**：
```
"error": "Channel is required when multiple channels are configured: feishu, openclaw-weixin"
```

### 教训
1. 有道云笔记编辑器结构复杂（多层iframe+contenteditable），agent-browser 无法正确写入 → 改用手动操作或有道API
2. 所有 cron job 的 delivery.channel 必须明确指定，不能用 "last"
