...
## Cron 定时任务超时问题（2026-03-30修复）

**问题根因**：isolated agentTurn 的 timeoutSeconds 太短，导致任务执行超时

| 任务 | 原超时 | 新超时 | 修复原因 |
|------|--------|--------|---------|
| daily-git-commit | 60秒 | 120秒 | git push 网络慢 |
| daily-youdao-summary | 180秒 | 600秒 | 浏览器自动化太慢 |
| morning-wechat-login-check | 30秒 | 120秒 | 登录命令阻塞等待 |

**修复方法**：`openclaw cron update <job-id> --patch '{"payload": {"timeoutSeconds": 600}}'`

**重要教训**：有道云笔记的编辑器结构太复杂（多层iframe+contenteditable），agent-browser 无法正确写入内容。定时任务中的 Youdao Note 操作需要预留足够长的时间，或者改为手动操作。
