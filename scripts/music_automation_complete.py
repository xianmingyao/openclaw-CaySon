...
def main():
    print("=== Smart Music Automation - COMPLETE v2 ===")
    ...
    # 5. 点击搜索框
    print("[Step 5] Clicking search box...")
    ...
    # 根据截图分析，歌曲中心位置在 (495, 305)
    song_x = 495
    song_y = 305
    print(f"[INFO] Clicking song at: ({song_x}, {song_y})")
    dc.click(song_x, song_y)
    time.sleep(1)
    dc.screenshot(os.path.join(SCREENSHOT_DIR, 'complete_step8_clicked.png'))
    ...
    dc.activate_window_by_process("QQMusic")
    time.sleep(3)
    dc.screenshot(os.path.join(SCREENSHOT_DIR, 'complete_final.png'))
