"""
智能音乐自动化脚本 v3 - 修复窗口激活问题
使用desktop-control技能，精确控制QQ音乐桌面端
"""
import pyautogui
import time
import os
...
def activate_window_by_title(title_part):
    ...
    try:
        ...
        if windows:
            win = windows[0]
            print(f"[DEBUG] Activating window: {win.title}")
            win.activate()
            time.sleep(1)
            # 如果窗口最小化，需要恢复
            if win.isMinimized:
...
def list_all_windows():
    ...
    try:
        ...
        print(f"[ERROR] List windows failed: {e}")
...
def search_and_play_desktop():
    ...
    # 1. 先列出所有窗口
    print("[DEBUG] Listing windows before launch...")
    list_all_windows()
    ...
    # 5. 按Tab键尝试聚焦到搜索框
    print("[Step 4] Trying to focus search box...")
    for i in range(5):
        pyautogui.press('tab')
        time.sleep(0.2)
    time.sleep(0.5)
    
    screenshot('desktop_step3_tabbed')
...
def main():
    print("=== Smart Music Automation v3 ===")
