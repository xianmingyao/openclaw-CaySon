    ...
    for attempt in range(retry):
        try:
            ...
            if resp.status_code == 200:
                ...
                log(f"    Rate limited, waiting {5*(attempt+1)}s...")
                time.sleep(5 * (attempt + 1))
                continue
            else:
                return {'success': False, 'error': result.get('message', 'Unknown')}
        except Exception as e:
            log(f"    Error: {str(e)[:50]}, retry {attempt+1}/{retry}")
            time.sleep(3)
    return {'success': False, 'error': 'Max retries exceeded'}

def main():
    log("=" * 50)
    log("NOTION FULL SYNC - Fixed")
    log("=" * 50)
    
    # 获取所有wiki文件
    wiki_files = list(WIKI_DIR.rglob("*.md"))
    total = len(wiki_files)
    log(f"Found {total} wiki files")
    ...
    for i, wiki_file in enumerate(wiki_files):
        ...
        if result.get('success'):
            ...
            log(f"    [FAIL] {result.get('error', 'Unknown')[:40]}")
            failed += 1
    ...
    log("")
    log("=" * 50)
    log(f"SYNC DONE: OK={success} FAIL={failed}")
