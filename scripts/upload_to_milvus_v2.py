...
def add_memory_to_milvus(milvus_client, text, metadata=None):
    ...
    try:
        ...
        memory_id = generate_id(text)
        
        data = [{
            "id": memory_id,
            "vector": embedding,
            "text": text[:4096],  # 截断到最大长度
            "user_id": USER_ID
        }]
        ...
        print(f"    [OK] Milvus 写入成功 (id={memory_id})")
        return True
...
def main():
    ...
    for i, item in enumerate(TODAY_MEMORIES, 1):
        ...
        if add_memory_to_milvus(milvus_client, item['memory'], {"category": item['category']}):
            success += 1
    
    print("\n" + "=" * 60)
    print(f"[完成] 成功上传 {success}/{len(TODAY_MEMORIES)} 条记忆到云端")
    print("=" * 60)
