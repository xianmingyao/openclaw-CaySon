...
│
└─ 不确定需要什么
   └─ → 运行 `python cli.py info` 查看所有功能
```

**请选择**（输入数字）：
1. 桌面操作自动化 → 使用 UFO 命令（mouse、keyboard、screenshot）
2. 浏览器网页自动化 → 使用 Bridge 命令（create、navigate、execute）
3. 智能技能生成/错误学习 → 使用 Evolution 命令（analyze、generate、execute）
4. 不确定 → 查看 `python cli.py info`

💡 **高级用户**：如果您已经熟悉 desktop-control，可以跳过此检查点直接进入 Phase 3。
...
**快速检查**（30秒内完成）：
- [ ] 数据库已初始化（`python cli.py evolution list-skills` 有输出）
- [ ] 目标应用/页面存在（手动确认可访问）
- [ ] 有管理员权限（UFO 操作需要）

💡 **如果以上任何一项不通过**，请参考"边界条件与 Fallback 策略"章节，或在 Phase 3 执行时使用 Fallback 方案。

**跳过此检查点**：如果您已完成环境准备并熟悉流程，可以直接进入 Phase 3。
...
| 检查项 | 验证方法 | 失败后果 |
|-------|---------|---------|
| **数据库已初始化** | `python cli.py evolution list-skills` 看到技能列表 | Evolution 功能不可用 |
...
# 强制关闭并重建会话
python cli.py bridge close session_123
...
```bash
# Step 1: 分析失败原因
python cli.py evolution analyze --execution-id exec_001 --tool-name ufo_click --error-type timeout

# Step 2: 生成修复技能
python cli.py evolution generate --name ufo_click_fixed --type FIX

# Step 3: 执行修复技能
python cli.py evolution execute --skill ufo_click_fixed
```
...
```
1. 启用详细日志
   └─ python cli.py --verbose [MODULE] [COMMAND]
...
```
检查点 1（Phase 1 完成）失败：
  症状：`python cli.py evolution list-skills` 报错或输出为空
  ├─ 原因：数据库未初始化
...
检查点 2（Phase 2 场景选择）失败：
  ...
  ├─ 原因：对自动化需求不明确
  ├─ 解决：
  │   1. 运行 `python cli.py info` 查看所有功能
...
- [ ] 已确认要使用的功能模块（UFO / Bridge / Evolution）
- [ ] 了解要操作的目标（屏幕坐标 / 网页URL / 技能名称）

**📌 快速导航**：
- **只想测试基本命令** → 查看"基本命令结构"（第538-587行）
- **需要完整业务场景** → 查看"完整业务场景示例"（京麦上架，第279-488行）
- **遇到问题** → 查看"常见错误和解决方案"（第1579-1688行）
