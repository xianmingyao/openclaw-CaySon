...
🤔 **检查点**：请确认您选择的自动化场景：
- 桌面操作自动化 → 使用 UFO 命令（mouse、keyboard、screenshot）
- 浏览器网页自动化 → 使用 Bridge 命令（create、navigate、execute）
- 智能技能生成/错误学习 → 使用 Evolution 命令（analyze、generate、execute）
- 如果不确定，可以先运行 `python cli.py info` 查看所有可用功能
...
在执行自动化任务前，了解可能的边界条件和 Fallback 策略，可以提高任务成功率。
