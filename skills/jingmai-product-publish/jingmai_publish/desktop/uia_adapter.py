...
class RealWindowsUIAAdapter:
    ...
    def _focus_detail_editor_body(self, handle: str) -> dict[str, object]:
        ...
        click_points.extend(
            [
                (window_rect.left + int(width * 0.405), window_rect.top + int(height * 0.815),
                 "ratio_code_body_bottom"),
    ...
    def _try_fill_dialog_filename(self, dialog: Any, resolved_path: str, dialog_handle: int | None = None) -> dict[
        str, object]:
        """优先命中文件名输入框和打开按钮。"""
            ...
            if self._is_dialog_still_open(dialog_handle):
                ...
                if self._try_click_dialog_file_item_and_submit(dialog, file_name, open_button, dialog_handle):
                    return {"success": True, "file_path": resolved_path,
                            "method": "filename_edit_then_file_item_submit"}
        ...
        if filename_edit is not None:
            try:
                ...
                if self._is_dialog_still_open(dialog_handle):
                    if self._try_click_dialog_file_item_and_submit(dialog, file_name, open_button, dialog_handle):
                        return {"success": True, "file_path": resolved_path}
                    return {"success": False, "file_path": resolved_path,
                            "error": "dialog_still_open_after_filename_submit"}
                return {"success": True, "file_path": resolved_path}
            except Exception:
                pass
