...
class JingmaiWorkflowService:
        ...
        if self._is_category_selection_document(document_text):
            ...
            if advanced_category_page:
                for _ in range(16):
                    time.sleep(0.5)
                    document_text = adapter.read_document_text(window_handle)
                    if self._extract_category_path_from_text(document_text) or self._is_publish_form_document(
                            document_text):
                        break
    ...
    def _fill_t4_required_attribute(self, window_handle: str, required_attribute: str) -> tuple[bool, str]:
        ...
        for value in candidates:
            ...
            time.sleep(0.3)
            document_text = adapter.read_document_text(window_handle)
            if value in document_text and "无数据" not in document_text and not re.search(r"类型\s*\*\s*请选择",
                                                                                          document_text):
                return True, value
    ...
    def run_t4_fill_additional_required_fields(
        ...
        if rated_voltage:
            actions.append(
                ("额定电压", adapter.select_combobox_by_label(window_handle, "额定电压", rated_voltage), rated_voltage))
        if cable_length:
            actions.append(
                ("电缆长度", adapter.select_combobox_by_label(window_handle, "电缆长度", cable_length), cable_length))
        if weight:
    ...
    def run_t6_upload_dialog_probe(self, window_handle: str, file_path: str) -> WorkflowStepResult:
        ...
        upload_result = self._complete_local_upload(window_handle, file_path) if trigger_ok else {"success": False,
    ...
    def run_t8_publish_product(self, window_handle: str, confirm_publish: bool = False) -> WorkflowStepResult:
        ...
        document_after = adapter.read_document_text(window_handle)
        review_visible = self._document_contains_any(document_after,
                                                     ["审核", "采销审核", "提交成功", "发布成功", "待审核"])
        state_changed = document_before != document_after
        screenshot_path = adapter.capture_window(window_handle)
    ...
    def _run_t6_upload_slot(
        ...
        if upload_strategy == "direct_click":
            upload_attempts = ["empty_slot_modal", "hover_local_upload", "hover_text_local_upload",
                               "text_index_fallback", "text_fallback"]
        elif upload_strategy == "text_fallback":
            upload_attempts = ["hover_text_local_upload", "text_index_fallback", "text_fallback", "empty_slot_modal",
                               "hover_local_upload"]
        else:
            upload_attempts = ["hover_local_upload", "hover_text_local_upload", "empty_slot_modal",
                               "occupied_slot_replace", "text_index_fallback", "text_fallback"]

        for attempt in upload_attempts:
            if upload_result.get("success"):
                break

            if attempt == "hover_local_upload" and empty_slot_action_index is not None and self._open_hover_local_upload(
                    window_handle, empty_slot_action_index):
                trigger_ok = True
                trigger_mode = "hover_local_upload"
                upload_result = self._complete_local_upload(window_handle, file_path,
                                                            slot_index=empty_slot_action_index)
                continue

            if attempt == "hover_text_local_upload" and empty_slot_action_index is not None and self._open_hover_local_upload_by_text(
                    window_handle, empty_slot_action_index):
                trigger_ok = True
            ...
            if attempt == "empty_slot_modal" and empty_slot_action_index is not None and hasattr(adapter,
                                                                                                 "click_image_upload_slot"):
            ...
            if attempt == "occupied_slot_replace" and before_image_count > slot_index and hasattr(adapter,
                                                                                                  "click_existing_image_slot"):
            ...
            if attempt == "text_index_fallback" and hasattr(adapter, "click_text_by_index"):
                for text in self._t6_upload_trigger_texts(slot_index):
                    ...
                    if trigger_ok:
                        ...
                        upload_result = self._complete_local_upload(window_handle, file_path,
                                                                    slot_index=empty_slot_action_index or 0)
                        break
                continue
    ...
    def _has_t6_slot_target(slots: list[dict[str, object]], *, slot_index: int) -> bool:
        ...
        image_slot_count = sum(
            1 for slot in slots if slot.get("status") == "filled" or slot.get("class_name") == "Image")
    ...
    def _resolve_t6_empty_slot_action_index(cls, slots: list[dict[str, object]], *, slot_index: int) -> int | None:
        empty_slots = [slot for slot in slots if
                       slot.get("status") == "empty" or slot.get("text") in cls.T6_EMPTY_SLOT_TEXTS]
        if not empty_slots:
            return None
    ...
    def _open_hover_local_upload(self, window_handle: str, slot_index: int) -> bool:
        ...
        if not hasattr(adapter, "hover_image_upload_slot"):
            return False
        slots = adapter.inspect_image_upload_slots(window_handle) if hasattr(adapter,
                                                                             "inspect_image_upload_slots") else []
        ...
        if hasattr(adapter, "click_text_near_bounds"):
            if adapter.click_text_near_bounds(window_handle, "本地上传", anchor_bounds=anchor_bounds, max_dx=520,
                                              max_dy=220):
                return True
    ...
    def _open_hover_local_upload_by_text(self, window_handle: str, slot_index: int) -> bool:
        ...
        for text in ["去设置", *self._t6_upload_trigger_texts(slot_index)]:
            if not adapter.hover_text_by_index(window_handle, text, slot_index):
                continue
            time.sleep(0.2)
            if hasattr(adapter, "click_local_upload_entry") and adapter.click_local_upload_entry(window_handle,
                                                                                                 slot_index):
                return True
    ...
    def _category_selection_candidates(document_text: str) -> list[str]:
        ...
        matched = re.search(r"近期使用类目：\s*(.+?)\s*下一步，完善其他商品信息", document_text, re.S)
        source = matched.group(1) if matched else document_text
        for matched_path in re.finditer(
                r"[\u4e00-\u9fffA-Za-z0-9、，/（）()]+(?:\s*>\s*[\u4e00-\u9fffA-Za-z0-9、，/（）()]+){2,}", source):
